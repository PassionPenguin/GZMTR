...
var imgexts = typeof imgexts == 'undefined' ? 'jpg, jpeg, gif, png, bmp, apng, svg' : imgexts;
var ATTACHORIMAGE = '0';
var STATUSMSG = {
    '-1': '内部服务器错误',
    '0': '上传成功',
...
function attachoption(type, op) {
    if (!op) {
        ...
        showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' + '<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />全选</label></span>' + '<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>确定</strong></button></p>', 'info', '未使用的' + (type == 'attach' ? '附件' : '图片'));
    } else if (op == 3) {

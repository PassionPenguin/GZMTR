...
window.initTheme = () => {
    GET_("dark") === "true" ? (window.isDark = true, document.documentElement.classList.add('dark-theme')) : android.getPref("theme") === "black" ? document.documentElement.classList.add('dark-theme') : document.documentElement.classList.add(android.getPref("theme") === "pink" ? "pink-theme" : android.getPref("theme") === "yellow" ? "yellow-theme" : android.getPref("theme") === "green" ? "green-theme" : android.getPref("theme") === "blue" ? "blue-theme" : "light-theme");
    if (window.matchMedia("(prefers-color-scheme: dark)").matches || document.documentElement.classList.contains("dark-theme")) {
        let r = $("img");
        for (let i = 0; i < r.length; i++) r[i].src.indexOf("white") !== -1 ? r[i].src = r[i].src.replace(/white/, "black") : r[i].src = r[i].src.replace(/black/, "white")
    }
};
window.cL = typeof android === "object" ? android.getPref("language") : "CHT";
window.l = ["CHT", "CHS", "JP", "KO", "EN"];
window.lN = ["繁體中文", "简体中文", "日本語", "한국어", "English"];
window.initLanguage = (type) => {
    if (type === "init") {
        if (typeof android !== "object") window.android = {
            getPref: function () {
                return ""
            }, storeStringPref: function (string, name) {
                cL = string
            }
        };
        let r = $("string[data-require='loadString']");
        for (let i = 0; i < r.length; i++) {
            if (r[i].getAttribute("data-stringid") === "cur") r[i].innerText = lN[l.indexOf(cL)]; else if (r[i].getAttribute("data-stringid") === "curTheme") r[i].innerText = android.getPref("theme") === "" ? f_name[27][l.indexOf(cL)] : f_name[Array("white", "black", "pink", "yellow", "green", "blue").indexOf(android.getPref("Theme")) + 27][l.indexOf(cL)]; else {
                let rd = f_name[parseInt(r[i].getAttribute("data-stringid"))][l.indexOf(cL)];
                if (rd.search("-") !== -1 && cL === "JP") {
                    let ru = cE("ruby");
                    ru.innerText = rd.split("-")[0];
                    let rt = cE("rt");
                    rt.innerText = rd.split("-")[1];
                    ru.appendChild(rt);
                    r[i].appendChild(ru)
                } else r[i].innerText = rd
            }
        }
    } else {
        let r = $("#LanguagesChoser li");
        for (let i = 0; i < r.length; i++) r[i].onclick = function () {
            android.storeStringPref("language", l[i]);
            $("#LanguagesChoser")[0].classList.remove("show");
            window.location.href = "Account.html"
        }
        let dr = $("#ThemeChoser li");
        for (let i = 0; i < dr.length; i++) dr[i].onclick = function () {
            android.storeStringPref("theme", Array("white", "black", "pink", "green", "yellow", "blue")[i]);
            $("#ThemeChoser")[0].classList.remove("show");
            window.location.href = "Account.html"
        }
    }
};
//
// window.saveRecentWatch = (num) => {
//     android.storeStringPref("data", android.getPref("data") + (android.getPref("data") === "" ? "" : ",") + num);
// };
//
// window.loadRecentWatch = () => {
//     console.log(android.getPref("data").split(","));
// }
...
window.fuckNeveadun = () => {
    ...
    res.length === 0 ? o.innerHTML = "<li>" + f_name[22][l.indexOf(cL)] + "</li>" : res.forEach(element => {
        let li = cE("li");
        let a = cE("span");
        let b = cE("span");
...
window.animateIt = (selector, name, type) => {
...
window.bounceOutRight = (selector) => {
    if (typeof (selector) === "object" && selector.length !== 1) {
        selector.forEach((e) => {
...
window.linedata = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "13", "14", "21", "GF", "APM"];
...
window.globallist = ["Line1-GZ", "Line2-GZ", "Line3-GZ", "Line4-GZ", "Line5-GZ", "Line6-GZ", "Line7-GZ", "Line8-GZ", "Line9-GZ", "Line13-GZ", "Line14-GZ", "Line21-GZ", "Line-GF", "LineAPM-GZ"];
window.openINF = (num, platnum) => {
